...
void Week3SineGeneratorAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    mSmoothedMod.setCurrentAndTargetValue(22.f);
...
void Week3SineGeneratorAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    for (int sample = 0; sample < buffer.getNumSamples(); sample++){
        ...
        float out_left = (asin(sinf(mod * phase * juce::MathConstants<float>::twoPi)) + asin(pow(sinf((mod_2 + 1) * phase * juce::MathConstants<float>::twoPi), .2))) * 0.125f;
...
void Week3SineGeneratorAudioProcessor::setMod(float inGain)
{
    mSmoothedMod.setTargetValue(inGain);
}
